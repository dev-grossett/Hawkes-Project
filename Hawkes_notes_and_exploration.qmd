...
function $L$ for $N$ is 
$$
L = \left[ \prod_{i=1}^{N_T} \lambda^*(t_i) \right] e^{-\Lambda(T)}
$$ {#eq-pp_likelihood}
and the log-likelihood function is
$$
l = \sum_{i=1}^{N_T} \log(\lambda^*(t_i)) - \Lambda(T)
$$ {#eq-pp_loglikelihood}
:::

### Example: Poisson Process Likelihood
...
plot(c(0, arrival_times), 0:length(arrival_times), type = "s", 
     xlab = "Time (t)", ylab = "Number of events N(t)",
     main = paste0("Simulated Poisson Process (λ = ", lambda, ")"))
...
cat(
  "Analytic Solution = ", 
  length(arrival_times)/T, "±", round(sqrt(length(arrival_times))/T, 3), "\n", 
  "Numeric MLE = ", fit$par, "±", round(1/sqrt(fit$hessian), 3)
)
```
