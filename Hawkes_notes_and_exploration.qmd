...
```{r}
# WIP library that currenly has my simulation and plotting functions
...
# simulate a poisson process over [0, T]
sim_poisson_process <- function(lambda, T, seed = NULL) {
  # set seed if given
  if (!is.null(seed)) {set.seed(seed)}
...
lambda <- 2
T <- 10
arrival_times <- sim_poisson_process(lambda, T, seed = 6)


plot_base_canvas(
  xlim = c(0, T*1.1), 
  ylim = c(0, length(arrival_times)*1.1), 
  title = paste0("Simulated Poisson Process (λ = ", lambda, ")"),
  ylab = "N(t)"
)
add_counting_process(arrival_times, T)

# log-likelihood of Poisson process arrival time data
poisprocess_loglik <- function(lambda, arrival_times, T) {
  N_T <- length(arrival_times)
  N_T*log(lambda) - lambda*T
}

# since optim minimises by default
poisprocess_negloglik <- function(lambda, arrival_times, T) {
  -poisprocess_loglik(lambda, arrival_times, T)
}
...
cat(sprintf(
  ...
  length(arrival_times)/T, 1.96*sqrt(length(arrival_times))/T, 
...
exp_hp_loglik <- function(theta, T, H_t) {
  # there is a speedup for the exponential hawkes log-likelihood that is O(N(T))
  # this is the more expensive O([N(T)]^2) computation
  ...
  sum(log_intensities) - exp_hp_compensator(theta = theta, t = T, H_t = H_t)
}

exp_hp_loglik_fast <- function(theta, T, H_t) {
  lambda <- theta[1]
  alpha <- theta[2]
  beta <- theta[3]
  ...
  sum(log_intensities) - exp_hp_compensator(theta, T, H_t)
}

exp_hp_negloglik<- function(theta, T, H_t, fast=TRUE) {
  if (fast) return(-exp_hp_loglik_fast(theta = theta, T = T, H_t = H_t))
  else return(-exp_hp_loglik(theta = theta, T = T, H_t = H_t))
}

T <- 50; lambda <- 0.5; alpha <- 2; beta <- 3;
set.seed(123)
t <- sim_hp(T = T, lambda = lambda, mu = function(t) {alpha*exp(-beta*t)})
