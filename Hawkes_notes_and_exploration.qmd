...
$$
l(\boldsymbol{\theta} \mid \mathbf{t}) = \sum_{i=1}^{N_T} \log(\lambda^*(t_i \mid
...
$$
l(\theta \mid (t_1,m_1),\ldots,(t_n,m_n),T) = \sum_{i=1}^n \log \lambda_g^*(t_i)
...
where $l_{\lambda}$ is the log-likelihood function for the ground intensity 
process, and $l_f$ is the log-likelihood function for the mark density.
$$
\begin{aligned}
l_{\lambda}(\theta_{\lambda} \mid (t_1,m_1),\ldots,(t_n,m_n),T) &= 
  \sum_{i=1}^n \lambda_g^*(t_i) - \int_0^T \lambda_g^*(s) \dd s \\
l_f(\theta_f \mid (t_1,m_1),\ldots,(t_n,m_n),T) &= 
